...
-2997 = -222-999-999-777 and here each digit is concatenated three times.
Write a function named checkConcatenatedSum/CheckConcatenatedSum that tests if a number has this generalized property.
*/

function checkConcatenatedSum(a,b) {
  return (""+Math.abs(a)).split('').map(e => e.repeat(b)).map(a => +a).reduce((a,b) => a+b) === Math.abs(a);
}
