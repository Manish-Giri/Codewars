...
function makesTheSentence(characterArray, sentenceString) {
  let map = {}, san = sentenceString.replace(/\s*/g, '').split('');
  san.forEach(e => map.hasOwnProperty(e) ? map[e] += 1: map[e] = 1);
  return san.every(s => checkCount(characterArray, s) >= map[s]);
}
const checkCount = (arr, i) => arr.filter(e => e === i).length;
